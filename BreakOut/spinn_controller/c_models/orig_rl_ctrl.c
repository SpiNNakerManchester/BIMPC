...
static bool initialize(uint32_t *timer_period)
{
  ...
  //log_info("bits4width: %d, bits4height: %d", bits_for_width, bits_for_height);
  //log_info("bat_x_start: %d, ball-y start: %d, ball-x start: %d,  action start: %d", bat_x_bit_start, ball_y_bit_start, ball_x_bit_start, action_bit_start);
  //log_info("Value space: %d", value_statespace_elements);
...
void timer_callback(uint unused, uint dummy)
{
   ...
   else
   {
      ...
      if ((ball_x != prev_ball_x) || (ball_y != prev_ball_y) || (bat_x != prev_bat_x)) {
         ...
         //log_info("Up: %d down: %d, diff: %k", score_up, score_down, recent_reward_or_punishment);
         score_up   = 0;
         score_down = 0;
         ...
         random_policy_choice = get_random_prob();
         if (random_policy_choice > 1.0 || random_policy_choice < 0.0) {
            bad_rnd++;
         }

         if (random_policy_choice < frac_of_random_moves) {
            chose = -1;
            // Some of the time, choose an action at random:
            //random_uint32 = (accum)sark_rand()>>16;
            random_num = get_random_prob();
            if (random_num < 0.4) {
               move_direction = KEY_LEFT;
               left_total++;
            } else if (random_num < 0.80) {
            ...
            move_count = 40+(int)(get_random_prob()*150);
         } else {
            chose = 1;
            // Most of the time, chose the action with the highest Q value:
            on_policy_count++;
            if ((q_left > q_right) && (q_left > q_none)) {
               move_direction = KEY_LEFT;
               left_total++;
            }
            else if ((q_right > q_left) && (q_right > q_none)) {
               move_direction = KEY_RIGHT;
               right_total++;
            }
            else { 
   ...
   if (outcount > 100000) {
      outcount = 0;
      log_info("Mvcount: %d, on-policy: %d, frac_rnd move: %k, Move: %d, Ball: %d:%d  Bat: %d   L: %d, R:%d, N:%d", 
      total_move_count, on_policy_count, frac_of_random_moves, move_direction, ball_x, ball_y, bat_x, left_total, right_total, none_total);
      log_info("Q-left: %k, Q-right: %k, Q-none: %k,  Q-inc: %k", q_left, q_right, q_none, q_increment);

      // Adjust fraction of random moves based on recent success:
      frac_success =  (accum)cum_score_up/cum_score_down;
      if (frac_success > frac_of_random_moves) {
         // Success! make random moves a little less likely:
         frac_of_random_moves -= 0.05 * (frac_of_random_moves - 0.05);
         
      } else {
         // Failure! make random moves a little more likely:
         frac_of_random_moves += 0.05 * (0.8 - frac_of_random_moves);
      }
      // Keep things open for now (not getting tied either to random or deterministic moves):
      if (frac_of_random_moves > 0.8)
         frac_of_random_moves = 0.8;
      else if (frac_of_random_moves < 0.05)
         frac_of_random_moves = 0.05;
   }
}
